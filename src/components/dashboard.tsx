...
import { useWorkspace, type WorkspaceUser } from "@/lib/use-workspace";
...
export function Dashboard({ user }: { user: WorkspaceUser }) {
  const workspace = useWorkspace(user);
  const { tasks, members, activity, organization } = workspace;
  ...
  return (
    <div className="app-shell">
      <AuthIdentitySync userId={user.id} />
      ...
      <aside className={`sidebar ${sidebarOpen ? "sidebar-open" : ""}`} aria-label="Main navigation">
        ...
        <div className="sidebar-bottom">
          ...
          <SignOutButton /><div className="profile"><Avatar member={owner} /><div><strong>{name}</strong><span>Signed in</span></div><span className="profile-dot" /></div>
        </div>
      </aside>
